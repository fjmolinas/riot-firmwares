...
static uint8_t bob_id[] = { 0xcc, 0xd1 };
static uint8_t alice_id[] = { 0xac, 0xe2 };
...
static void test_security_ctx_key_gen(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, memcmp(ctx.send_ctx_key, expected_send_key, SECURITY_CTX_KEY_LEN));
    TEST_ASSERT_EQUAL_INT(0, memcmp(ctx.recv_ctx_key, expected_recv_key, SECURITY_CTX_KEY_LEN));
...
static void test_security_ctx_gen_nonce(void)
{
    ...
    security_ctx_init(&ctx, alice_id, sizeof(alice_id), bob_id, sizeof(bob_id));
    security_ctx_key_gen(&ctx, salt, sizeof(salt), secret, sizeof(secret));
    ctx.seqnr = expected_nonce_seqnr;
    security_ctx_gen_nonce(&ctx, alice_id, sizeof(alice_id), nonce);
...
static void test_security_ctx_encode_decode(void)
{
    ...
    uint8_t *out;
    size_t olen = security_ctx_encode(&bob_ctx, (uint8_t*) message, sizeof(message), buf, sizeof(buf), &out);
